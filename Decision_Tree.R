...
no_dumtrain <- trainset[-c(2,4,7)]
no_dumtest <- testset[-c(2,4,7)]
dim(no_dumtrain)
dim(no_dumtest)
...
#Predict on testset
rpart_train_pred <- predict(rpart_model, no_dumtest)
confusionMatrix(no_dumtest$fraud, rpart_train_pred,positive = "Yes")
#Plot ROC
library(pROC)
rpart_model_prob <- predict(rpart_model, no_dumtest, type = "prob")
rpart_model_ROC <- roc(response = no_dumtest$fraud, predictor = rpart_model_prob$Yes,
                       levels = levels(no_dumtest$fraud))
plot(rpart_model_ROC, col = "red")

auc(rpart_model_ROC)

#################################################
#### Removing dummy variables downsample    ####
################################################
...
trainset_DS <- trainset_DS[-c(2,4,7)]
testset_DS <- testset_DS[-c(2,4,7)]
...
#model using C50
c50model <- C5.0(trainset_DS[-7], trainset_DS$fraud)
c50model

summary(c50model)
#validation of C50
c50model_pred <- predict(c50model, testset_DS)
CrossTable(testset_DS$fraud, c50model_pred,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('Actual Fraud','Predicted Fraud'))
confusionMatrix(testset_DS$fraud, c50model_pred)
#Boosting C50 model
fraud_boost <- C5.0(trainset_DS[-7], trainset_DS$fraud, trials = 10)
fraud_boost

summary(fraud_boost)
#validation of boosting c50 model
fraud_boost_pred <- predict(fraud_boost, testset_DS)
CrossTable(testset_DS$fraud, fraud_boost_pred,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('Actual Fraud','Predicted Fraud'))
confusionMatrix(testset_DS$fraud, fraud_boost_pred, positive = "Yes")
